...
def check_recent_aa30c_erep():
    sb = get_supabase()
    
    # Get positions from the last 2 days with rule Aa30c
    print("\n--- RECENT Aa30C POSITIONS ---")
    positions = sb.table('positions').select('*').ilike('rule_code', '%Aa30C%').order('id', desc=True).limit(50).execute()
    ...
    res = sb.table('strategy_rules_v2').select('*').eq('rule_code', 'Aa30C').execute()
    if res.data:
        rule = res.data[0]
