...
async def fetch_bb21():
    sb = get_supabase()
    res = sb.table('strategy_rules_v2').select('*').eq('rule_code', 'Bb21').execute()
    if not res.data:
        print("Bb21 not found in DB.")
        return
        
    rule = res.data[0]
    print('Rule details for Bb21:', json.dumps({k:v for k,v in rule.items() if k not in ['created_at', 'updated_at']}, indent=2))
