...
async def get_dashboard_summary():
    ...
    for attempt in range(max_retries):
        try:
            ...
            err_str = str(e).lower()
            is_transient = "disconnect" in err_str or "timeout" in err_str or "connection" in err_str
            if attempt < max_retries - 1 and is_transient:
                import asyncio
                await asyncio.sleep(0.25)
                continue
            
            # Si es un fallo transitorio de sondeo HTTP, no spamear Telegram con alarmas de emergencia
            from app.core.logger import logger
            logger.warning(f"[DASHBOARD_API] Transient summary poll error: {e}")
            return {
