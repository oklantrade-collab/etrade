    ...
    <div style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', background: 'rgba(0,0,0,0.9)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, backdropFilter: 'blur(8px)' }}>
      <div style={{ background: '#0F1117', width: '95%', maxWidth: '1100px', borderRadius: '24px', border: '1px solid #22C55E', overflow: 'hidden', maxHeight: '95vh', display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ padding: '24px', overflowY: 'auto' }}>
            <h3 style={{ fontSize: '13px', fontWeight: 950, color: '#22C55E', marginBottom: '15px' }}>🔵 SUSTENTACIÓN TÉCNICA {isPro ? '(PRO - 1 DÍA)' : '(HOT - MTF)'}</h3>
            {isPro ? (
                 <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '10px' }}>
                    <RuleBox id="P01" name="EMA50>200" val={score >= 40 ? 'OK' : 'FAIL'} pts={40} pass={score >= 40} />
                    <RuleBox id="P02" name="EMA20>50" val={score >= 70 ? 'OK' : 'FAIL'} pts={30} pass={score >= 70} />
                    <RuleBox id="P03" name="SAR 1D" val={score >= 90 || (score > 10 && score < 70) ? 'OK' : 'FAIL'} pts={20} pass={score >= 90 || (score > 10 && score < 70)} />
                    <RuleBox id="P04" name="RSI <= 30" val={(score % 10 !== 0) ? 'OK' : 'FAIL'} pts={10} pass={(score % 10 !== 0)} />
                 </div>
            ) : (
