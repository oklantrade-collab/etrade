...
export default function ForexStrategiesPage() {
  ...
  async function loadRules() {
    try {
      ...
      if (res.ok) {
        let allRules: StrategyRule[] = await res.json()
        // Filtrar solo reglas aplicables a Forex
        setRules(allRules.filter(r => r.market_types?.includes('forex_futures')))
      }
    } catch (err) {
  ...
  return (
    <div className="strategies-page space-y-8 pb-20 p-8 min-h-screen bg-[#020617] text-white font-sans">
      {/* HEADER */}
      <div className="flex justify-between items-center bg-[#0F172A]/50 p-6 rounded-2xl border border-white/5 backdrop-blur-xl">
        <div className="flex flex-col gap-1">
          <div className="flex items-center gap-2 text-[0.65rem] font-black text-slate-500 uppercase tracking-widest">
            <Link href="/forex/dashboard" className="hover:text-blue-400 transition-all">Forex</Link>
            <span>/</span>
            <span className="text-slate-300">Strategies</span>
          </div>
          <h1 className="text-3xl font-black italic tracking-tighter text-white">Forex Engine <span className="text-blue-500">v1.2</span></h1>
        </div>
        
        <div className="flex items-center gap-6">
           <div className="text-right">
              <span className="text-[0.6rem] text-slate-500 uppercase font-black block leading-none mb-1 tracking-tighter">Forex Rules Active</span>
           ...
           <div className="flex flex-col items-end">
              <span className="text-[0.6rem] text-slate-500 uppercase font-black block leading-none mb-1 tracking-tighter">IC Markets Server</span>
              <span className="text-xs font-black text-blue-400 bg-blue-500/10 px-2 py-1 rounded">LIVE SYNC</span>
           </div>
        </div>
      </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
               {filteredScalping.length > 0 ? (
                 ...
                 <div className="col-span-full py-20 text-center text-slate-500 italic text-sm border-2 border-dashed border-slate-800 rounded-3xl">
                    No hay reglas de {activeDir} para Forex Scalping.
                 </div>
               )}
            </div>
          </div>
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
              ...
              {filteredSwing.length === 0 && (
                <div className="col-span-full py-20 text-center text-slate-500 italic text-sm border-2 border-dashed border-slate-800 rounded-3xl">
                   No hay reglas de {activeDir} para Forex Swing Trade.
                </div>
              )}
            </div>
          </div>
      ...
      <div className="relative z-10 space-y-4">
        ...
        <div>
          ...
          <p className="text-[0.65rem] text-slate-500 line-clamp-2 italic">
            {rule.notes || "Análisis dinámico de IC Markets."}
          </p>
        </div>
        ...
        return (
          <div key={s} className="bg-[#0F172A]/80 border border-white/5 p-6 rounded-[2rem] space-y-6 relative overflow-hidden group">
            <div className="flex justify-between items-center">
              <h4 className="text-xl font-black italic tracking-tighter text-white">{s}</h4>
              <div className="flex items-center gap-2">
              ...
              <div className="space-y-6">
                ...
                <div className="space-y-2">
                  <div className="flex justify-between items-end">
                    ...
                    <span className={`text-2xl font-black font-mono transition-all tabular-nums ${evalData.best_opportunity.score >= evalData.best_opportunity.min_score ? 'text-emerald-400' : 'text-white'}`}>
                      {evalData.best_opportunity.score.toFixed(2)}
                    </span>
                  </div>
                  <div className="h-2 w-full bg-slate-800 rounded-full overflow-hidden">
                    <div 
                      className={`h-full transition-all duration-1000 ${evalData.best_opportunity.score >= evalData.best_opportunity.min_score ? 'bg-emerald-500 shadow-[0_0_15px_rgba(16,185,129,0.5)]' : 'bg-slate-600'}`} 
