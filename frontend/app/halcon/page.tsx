  ...
  return (
    <div className="space-y-8 pb-20 p-6">
      ...
      <div className="card glass-effect-premium border-white/5 p-6 relative overflow-hidden flex justify-between items-center">
        <div className="flex items-center gap-6">          <div className={`w-4 h-4 rounded-full ${status.enabled ? 'bg-emerald-500 animate-pulse' : 'bg-rose-500'}`} />
          <div>
      ...
      {/* Grid: Scores + Decisions */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        {/* Recent Scores */}
        <div className="card glass-effect border-slate-800/50 p-6">
          <div className="flex justify-between items-center mb-6">
            <h3 className="text-sm font-black text-slate-400 uppercase tracking-widest">Evaluaciones Recientes</h3>
            <span className="text-xs">📡</span>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-white/5 text-[0.65rem] font-black text-slate-500 uppercase tracking-wider">
              ...
              <tbody>
                {scores.map((score, i) => (
                  <tr key={i} className="border-b border-white/5 hover:bg-white/5 transition-all text-xs">
                    <td className="py-3 px-2 font-bold">{score.symbol}</td>
                    <td className={`py-3 px-2 font-black ${score.direction === 'long' ? 'text-emerald-400' : 'text-rose-400'}`}>
                      {score.direction?.toUpperCase()}
                    </td>
                    <td className="py-3 px-2 font-mono">{score.score_final?.toFixed(1)}</td>
                    <td className="py-3 px-2">
                      <span className={`px-2 py-0.5 rounded text-[0.65rem] font-black ${
                        score.semaforo === 'VERDE' ? 'bg-emerald-500/10 text-emerald-400' :
                        score.semaforo === 'AMARILLO' ? 'bg-amber-500/10 text-amber-400' : 'bg-rose-500/10 text-rose-400'
                      }`}>
                        {score.semaforo}
                      </span>
                    </td>
                    <td className="py-3 px-2 text-[0.7rem] font-bold text-slate-400">{score.decision}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="space-y-8">
          ...
          <div className="card glass-effect border-slate-800/50 p-6">
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-sm font-black text-slate-400 uppercase tracking-widest">Decisiones Ejecutadas</h3>
              <span className="text-xs">⚡</span>
            </div>
            <div className="overflow-x-auto">
                ...
                <tbody>
                  {decisions.map((dec, i) => (
                    <tr key={i} className="border-b border-white/5 hover:bg-white/5 transition-all">
                      ...
                      <td className="py-3 px-2 text-[0.65rem] text-slate-400 truncate max-w-[150px]">{dec.reason}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <div className="card glass-effect border-slate-800/50 p-6">
            ...
            <div className="space-y-3">
              {events.map((evt, i) => (
                <div key={i} className="flex justify-between items-center bg-slate-900/50 p-3 rounded-lg border border-white/5">
                  <div>
                    <div className="text-xs font-bold text-white mb-1">{evt.event}</div>
                    <div className="flex gap-2 text-[0.6rem] font-black uppercase tracking-wider">
                      <span className="text-slate-400">{evt.currency}</span>
                      <span className="text-slate-600">•</span>
                      <span className={`${evt.impact === 'HIGH' ? 'text-rose-400' : 'text-amber-400'}`}>{evt.impact}</span>
                    </div>
                  </div>
                  <div className="text-right">
                    <div className="text-sm font-black text-blue-400 font-mono">{new Date(evt.event_datetime).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</div>
                    <div className="text-[0.55rem] text-slate-500 uppercase font-black">Pause: ±{evt.pause_window_minutes}m</div>
                  </div>
                </div>
              ))}
            </div>
      ...
      <button 
        ...
        className="fixed bottom-8 right-8 w-14 h-14 bg-indigo-600 hover:bg-indigo-500 text-white rounded-full flex items-center justify-center shadow-2xl transition-all cursor-pointer z-40 border border-indigo-400/30 hover:scale-110"
      >
